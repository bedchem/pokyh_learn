...
export default async function TeamsPage() {
  const { token, identity } = await requireLearnAdministrator('/teams');
  const teams = await getTeams(token).catch(() => []);
  return <AppShell initialIdentity={identity}><div className="page-wrap"><section className="page-heading page-heading--inline"><div><p className="eyebrow"><Text id="teams.eyebrow" /></p><h1><Text id="teams.title" /></h1><p className="page-lead"><Text id="teams.body" /></p></div></section>{teams.length ? <TeamsView teams={teams} /> : <EmptyState title={<Text id="teams.emptyTitle" />} body={<Text id="teams.emptyBody" />} href="/teams/new" action={<Text id="teams.create" />} />}</div></AppShell>;
}
