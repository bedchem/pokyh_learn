...
| `LEARN_BFF_AI_BODY_LIMIT_BYTES` | optional | larger body limit for `/api/learn/ai/*` (defaults to 20MB) to fit base64-encoded chat attachments; the backend's `LearnAiConfig.uploadMaxBytes` remains the authoritative per-file limit |
