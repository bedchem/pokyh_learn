...
export function AuthForm({ legalConfig }: { legalConfig: LearnLegalConfig }) {
  ...
  return (
    <form className="auth-form" onSubmit={submit}>
      ...
      <label>{t('auth.password')}<span className="password-field"><span className="input-icon"><LockKeyhole size={17} /></span><input id="learn-password" name="password" type={visible ? 'text' : 'password'} autoComplete="current-password" minLength={1} maxLength={200} required placeholder="••••••••" /><button className="password-toggle" type="button" onClick={() => setVisible((value) => !value)} aria-controls="learn-password" aria-pressed={visible} aria-label={visible ? t('auth.hidePassword') : t('auth.showPassword')}>{visible ? <EyeOff size={17} /> : <Eye size={17} />}</button></span></label>
